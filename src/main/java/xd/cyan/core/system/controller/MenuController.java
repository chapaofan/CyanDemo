...
import xd.cyan.core.common.entity.MenuTree;
import xd.cyan.core.common.exception.FebsException;
import xd.cyan.core.system.entity.Menu;
import xd.cyan.core.system.entity.User;
import xd.cyan.core.system.service.IMenuService;
...
public class MenuController extends BaseController {
    ...
    @GetMapping("{username}")
    public CyanResponse getUserMenus(@NotBlank(message = "{required}") @PathVariable String username) throws FebsException {
        ...
        return new CyanResponse().data(userMenus);
    }

    @GetMapping("tree")
    @ControllerEndpoint(exceptionMessage = "获取菜单树失败")
    public CyanResponse getMenuTree(Menu menu) {
        MenuTree<Menu> menus = this.menuService.findMenus(menu);
        return new CyanResponse().success().data(menus.getChilds());
    }

    @PostMapping
    @RequiresPermissions("menu:add")
    @ControllerEndpoint(operation = "新增菜单/按钮", exceptionMessage = "新增菜单/按钮失败")
    public CyanResponse addMenu(@Valid Menu menu) {
    ...
    public CyanResponse deleteMenus(@NotBlank(message = "{required}") @PathVariable String menuIds) {
    ...
    public CyanResponse updateMenu(@Valid Menu menu) {
